...
const Login = () => {
  ...
  const onLoginSubmit = async () => {
    console.log("click evenets");

    try {
      setLoading(true);
      if (email) {
        const response = await sendOtp(null, null, email);
        console.log("Response from email sendOtp:", response);
          ...
          step === 1 && (
            <form className='space-y-4' onSubmit={handleLoginSubmit(onLoginSubmit)}>
              ...
              <div className='relative'>
                <div className='flex'>
                  <div className='relative w-1/3'>
                    <button type='button' className={`flex-shrink-0 z-10 inline-flex items-center py-2.5 px-4 text-sm font-medium text-center 
                            ${theme === 'dark' ? 'bg-gray-700 text-white border-gray-600' : 'bg-gray-100 text-gray-900 border-gray-300'} border rounded-s-lg hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100`}
                    ...
                    {
                      showDropdown && (
                        <div className={`absolute z-20 w-full mt-1 ${theme === 'dark' ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-300'} border rounded-md shadow-lg max-h-60 overflow-auto`}>
                          <div className={`sticky top-0 ${theme === 'dark' ? 'bg-gray-700' : 'bg-white'} p-2`}>
                            <input type="text" placeholder="Search Countries..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className={`w-full border px-2 py-1 ${theme === 'dark' ? 'bg-gray-600 border-gray-500 text-white' : 'bg-white border-gray-300'} rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-green-500`} />
                          </div>
                          {
                            filterCountries.map((country) => (
                              <button key={country.alpha2} type='button' className={`flex w-full text-left px-3 py-2 ${theme === 'dark' ? 'hover:bg-gray-600' : 'hover:bg-gray-100'} focus:outline-none focus:bg-gray-100`}
                  ...
                  <input type="text" placeholder='Phone Number' {...loginRegister("phoneNumber")} value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)}
              ...
              <div className={`flex items-center border rounded-md px-3 py-2 ${theme === 'dark' ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-300'}`}>
                ...
                <input type="email" placeholder='Email Address(Optional)' {...loginRegister("email")} value={email} onChange={(e) => setEmail(e.target.value)}
...

        {
          step === 2 && (
            <form onSubmit={handleOtpSubmit(onOtpSubmit)} className='space-y-4'>
              <p className={`text-center ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}mb-4`}>Please Enter the 6-digit OTP send to Your {userPhoneData ? userPhoneData.phoneSuffix : "Email"} {" "}
                {userPhoneData.phoneNumber && userPhoneData?.phoneNumber}</p>

               {/* 6 input box wil generate from here */}
              <div className='flex justify-between'>
                {otp.map((digit, index) => (
                  <input key={index} id={`otp-${index}`} type="text" maxLength={1} value={digit} onChange={(e) => handleOtpChange(index, e.target.value)}
              ...
              <button type='button' onClick={handleBack} className={`w-full mt-2 ${theme === 'dark' ? 'bg-gray-700 text-gray-300' : 'bg-gray-200 text-gray-700'} py-2 rounded-md hover:bg-gray-300 transition flex justify-center items-center`}>
            ...
            <form onSubmit={handleProfilesubmit(onProfileSubmit)} className='space-y-4'>
              <div className='flex flex-col items-center mb-4'>
                  <div className='relative w-24 h-24 mb-2'>
                    <img src={profilePicture || selectedAvatar} alt="Profile Picture" className='w-full h-full object-cover rounded-full'/>
                    <label htmlFor='profile-picture' className='absolute bottom-0 right-0 bg-green-500 text-white rounded-full p-2 cursor-pointer hover:bg-green-600 transition'>
                      <FaPlus/>
                    </label>
                    <input type="file" id="profile-picture" accept='image/*' className="hidden" onChange={handleFileChange} />
                  </div>
                  <p className={`text-lg font-bold ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'} mb-2`}>
                    Choose an Avatar
                  </p>
              ...
              <div className='relative'>
                  <FaUser className={`absolute top-1/2 left-3 transform -translate-y-1/2 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'}`} />
                  <input {...profileRegister("username")} type='text' placeholder='Username' 
                  className={`w-full pl-10 pr-3 py-2 border rounded-md text-lg ${theme === 'dark' ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300'} focus:outline-none focus:ring-2 focus:ring-green-500`} />
                  {profileErrors.username && <p className="text-red-500 text-sm mt-1">{profileErrors.username.message}</p>}
              </div>

              <div className='flex items-center space-x-2'>
                    <input {...profileRegister('agreed')} type='checkbox' className={`rounded ${theme === 'dark' ? 'bg-gray-700  text-green-500' : 'text-green-500'} focus:ring-2 focus:ring-green-500`} />
              ...
              <button type='submit'  className={`w-full bg-green-500 text-white font-bold py-2 rounded-md hover:bg-green-600 transition duration-300 ease-in-out transform hover:scale-105 flex items-center justify-center text-lg
